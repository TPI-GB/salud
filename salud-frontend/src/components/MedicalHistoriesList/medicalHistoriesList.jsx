...
export default function MedicalHistoriesList(props) {
    ...
    return (
        <List sx={{minWidth:300, maxWidth:500, ml:"auto", mr:"auto", mt:1}}>
            {mhList.map((medicalHistory) =>
                <ListItem key={medicalHistory._id} sx={{border:"2px solid", borderRadius:"25px"}}>
...
function RenderMedicalHistory(props) {
    ...
    return (
        <>
            ...
            <Fab color="secondary" size="small" component={Link} to={`/HistoriaClinica/${mh._id}`}>
                <ForwardIcon />
            </Fab>
        </>
    )
